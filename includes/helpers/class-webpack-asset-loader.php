 ...
 */
class Webpack_Asset_Loader implements Asset_Loader_Interface {
	 ...
	 * @param array  $dependencies Handles of the JS files this file depends on.
	 * @return void
	 */
	public function enqueue_script( string $handle, array $dependencies = array() ): void {
		$js_dependencies = $this->get_js_dependencies( $handle, $dependencies );
		wp_enqueue_script(
	 ...
	 */
	private function get_js_dependencies( string $handle, array $dependencies = array() ): array {
		...
		if ( isset( $asset_data['dependencies'] ) ) {
			$script_dependencies = array_merge( $script_dependencies, $asset_data['dependencies'] );
		}
	 ...
	 * @param string $handle Optional custom handle. If empty, one will be generated.
	...
	public function register_script( string $path, string $handle = '', array $deps = array() ): string {
		...
		$prefixed_handle = $handle ? $handle : $this->get_file_handle( $basename );
	...
	public function register_style( string $path, string $handle = '', array $deps = array() ): string {
		$unversioned = $this->get_unversioned_path( $path );
		if ( ! isset( $this->manifest->{$unversioned} ) ) {
			return $path;
		}
		if ( isset( $this->registered_styles[ $unversioned ] ) ) {
			return $this->registered_styles[ $unversioned ];
		}
		$prefixed_handle = $handle ? $handle : $this->get_file_handle( $path );
		wp_register_style(
			$prefixed_handle,
			$this->get_resource_uri( $path ),
			$deps,
			$this->get_resource_version( $path ),
			$this->default_css_media
		);
		$this->registered_styles[ $unversioned ] = $prefixed_handle;
		return $prefixed_handle;
	}
	...
	private function get_resource_version( string $handle ): ?string {
		...
		if ( isset( $this->manifest->{$handle} ) ) {
			...
			return (string) filemtime( $resource_path );
		}
